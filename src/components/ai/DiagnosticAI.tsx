      ...
      <DialogContent className="sm:max-w-[600px] bg-card text-card-foreground">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-foreground">Síntomas</FormLabel>
                  <FormControl>
                    <Textarea
                      placeholder="p. ej., 'El motor falla al acelerar, humo blanco por el escape y un ruido agudo de la transmisión...'"
                      className="min-h-[120px] bg-card text-card-foreground border-black/30"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
        ...
        {result && (
          <div className="mt-4 rounded-lg border border-black/20 bg-muted/50 p-4">
            <h3 className="font-semibold mb-2 text-foreground">Sugerencias:</h3>
            <p className="text-sm whitespace-pre-wrap text-muted-foreground">{result}</p>
          </div>
        )}
      </DialogContent>
    </Dialog>
