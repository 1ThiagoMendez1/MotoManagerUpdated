...
if (code.includes(searchStr)) {
  ...
  // Also append depositHistory to the return object
  const returnStr = `    assignedTechnician: wo.assigned_mechanic_id ? {
      id: wo.assigned_mechanic_id,
      name: technicians.find(t => t.id === wo.assigned_mechanic_id)?.name || 'Técnico desconocido',
    } : undefined,`;
    
  if (code.includes(returnStr)) {
      code = code.replace(returnStr, returnStr + "\n    depositAmount: parsedDeposit,\n    depositHistory: depositHistory,");
